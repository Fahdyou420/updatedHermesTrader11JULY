"""Local SMA100 + stochastic scalping backtester for XAUUSD M15."""
import json
...
OUT = Path(r'C:/Users/user/Desktop/hermes_claude/data/rnd/results/strategy_sma_stochastic_M15.json')
...
def stochastic(high, low, close, k=5, d=3, smooth=3):
    ...
    for i in range(d - 1, n):
        stoch_d[i] = np.nanmean(stoch_k[i - d + 1:i + 1])
...
def run():
    ...
    for i in range(110, n):
        ...
        if np.isnan(sma100[i]) or np.isnan(k_line[i]):
            continue
        curr_close = float(close[i])
        curr_open = float(open_[i])
        if curr_close <= sma100[i]:
            continue
        support_bounce = (low[i] <= sma100[i] <= high[i]) or (low[i] < sma100[i] < curr_close)
        stoch_turn_up = (k_line[i - 1] <= 20) and (k_line[i] > 20) if i > 0 else False
        if not (support_bounce and stoch_turn_up):
            continue
        prev_low = float(min(low[max(0, i - 5): i]))
        sl = min(prev_low, float(low[i - 1]))
        risk = curr_close - sl
        if risk <= 0:
            continue
        tp = curr_close + 1.8 * risk
        trade = {'entry_idx': i, 'entry': curr_close, 'sl': sl, 'tp': tp, 'rr': 1.8, 'bars': 0}
        in_trade = True
    ...
    out = {
        'strategy_id': 'strategy_sma_stochastic_M15',
        'symbol': 'XAUUSD',
        'timeframe': 'M15',
        'total_trades': total,
        'win_rate': win_rate,
